...
class Installer:
    def __init__(self):
        ...
        self.grocy_config_path = join(self.data_dir, 'grocy', 'config')
        self.grocy_config_file = join(self.grocy_config_path, 'config.php')
    ...
    def install_config(self):
        ...
        fs.makepath(self.grocy_config_path)
    ...
    def install(self):
        ...
        default_config_file = join(self.config_dir, 'config-dist.php')
        shutil.copy(default_config_file, self.grocy_config_file)
    ...
    def post_refresh(self):
        self.install_config()
        self.migrate_grocy_config_file()
    ...
    def migrate_grocy_config_file(self):
        if not isfile(self.grocy_config_file):
            old_grocy_config_file = join(self.common_dir, 'grocy', 'config', 'config.php')
            if isfile(old_grocy_config_file):
                shutil.copy(old_grocy_config_file, self.grocy_config_file)
                self.fix_config_permission()

    def fix_config_permission(self):
        fs.chownpath(self.grocy_config_file, USER_NAME)

    def fix_version_specific_dbhost(self):
        # Nextcloud-specific?
        content = self.read_grocy_config()
        pattern = r"'dbhost'\s*=>\s*'.*?'"
        replacement = "'dbhost' => '{0}'".format(self.db.database_host)
        new_content = re.sub(pattern, replacement, content)
        self.write_grocy_config(new_content)
        self.fix_config_permission()

    def read_grocy_config(self):
        with open(self.grocy_config_file) as f:
            return f.read()

    def write_grocy_config(self, content):
        with open(self.grocy_config_file, "w") as f:
            f.write(content)
...
# 
        ...
        # # else:
        # #     self.log.info('not upgrading grocy')
